...
class data_preprocessor:
    ...
    def parse_one_file(self, fname, dictionary, use_chars):
        ...
        doc_sent_raw = [s.strip().split() for s in sent_tokenize(raw[2])]
        ...
        #qry_raw[cloze] = ans_raw

        neg_raw = copy.copy(qry_raw)
       
        neg_answer = random.choice(cand_raw)[0]
        
        if neg_answer==ans_raw:
            neg_answer = random.choice(cand_raw)[0]
        if neg_answer==ans_raw:
            neg_answer = random.choice(cand_raw)[0]
        
        neg_raw[cloze] = neg_answer
        # tokens/entities --> indexes
        doc_words = list(map(lambda w: w_dict.get(w, 0), doc_raw))
        doc_sent_words = [list(map(lambda w: w_dict.get(w, 0), s)) for s in doc_sent_raw]
        for i in enumerate(doc_sent_words):
            if len(i[-1]) > 150:
                tmp = doc_sent_words.pop(i[0])
                a = len(tmp) // 150
                b = len (tmp) % 150
                for ii in range(150-b):
                    tmp.append(0)
                tmp = np.array(tmp)
                tmp = tmp.reshape(-1,150)
                tmp = tmp.tolist()
                for ii in range(a+1):
                    doc_sent_words.insert(i[0]+ii, tmp[ii])

        qry_words = list(map(lambda w: w_dict.get(w, 0), qry_raw))
        neg_words = list(map(lambda w: w_dict.get(w, 0), neg_raw))
        if use_chars:
            doc_chars = list(
        ...
        return doc_words, doc_sent_words, qry_words,neg_words, ans, cand, \
            doc_chars, qry_chars, cloze, fname
